...
function main(dataUrl) {
	if (canvas.getContext) {
		...
		image.addEventListener('load', function(){
			...
			ctx.save();
			ctx2.drawImage(image, 0, 0, repWidth, repHeight);
			ctx2.save();
			//trimRestore();
			$("#js_triming_area").css({
				width: repWidth,
...
$(function(){
	...
	// var left = elmX;
	// var top  = elmY;
	// $hammerObj.on("dragstart",function(event) {
	// 	dragStartX = event.gesture.center.pageX;
	// 	dragStartY = event.gesture.center.pageY;
	// });
	// $hammerObj.on("drag",function(event) {
	// 	left += (event.gesture.center.pageX - dragStartX);
	// 	top += (event.gesture.center.pageY - dragStartY);
	// 	if (top > 0) {
	// 			top = 0;
	...
	$hammerObj.on("pan",function(event) {
      if(event.isFinal) { //end
          ...
          $jqIdTrimingArea.data("down", false);
					console.log(elmX);
					ctx2.clearRect(0,0,repWidth, repHeight);
					//ctx2.drawImage(image, 0, 0, repWidth, repHeight, elmX, elmY, repWidth, repHeight);
					ctx2.drawImage(image, elmX, elmY, repWidth, repHeight);
					imageData = ctx2.getImageData(0,0,repWidth, repHeight);
